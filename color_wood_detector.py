...
class ColorWoodDetector:
    def __init__(self):
        self.wood_color_profiles = {
            ...
            'bottom_panel': {
                'hsv_lower': np.array([110, 15, 100]),  # Expanded minimum HSV values for the bottom panel
                'hsv_upper': np.array([150, 25, 100]),  # Tightened maximum HSV values for the bottom panel to exclude bright white regions
        ...
        self.contour_approximation = 0.02 # Epsilon for contour approximation
    ...
    def calculate_width_mm(self, bbox_pixels: int, camera: str = 'top') -> float:
        ...
        if camera == 'top':
            ...
            raise ValueError("Camera must be 'top'")
    ...
    def visualize_detection(self, image: np.ndarray, detection_result: Dict) -> np.ndarray:
        ...
        for i, candidate in enumerate(detection_result['wood_candidates']):
            ...
            # Draw corner points for 4-point detection
            corner_points = candidate['corner_points']
            cv2.drawContours(vis_image, [corner_points], -1, (255, 0, 0), 3)
...
def main():
    ...
    # Define center ROIs (top camera 60% height, full width, centered; bottom camera 60% of frame, centered)
    frame_width = 1280
    frame_height = 720
    roi_height_top = int(frame_height * 0.6)
    roi_y_top = (frame_height - roi_height_top) // 2
    roi_width_top = frame_width
    roi_x_top = 0
    roi_top = (roi_x_top, roi_y_top, roi_width_top, roi_height_top)

    roi_width_bottom = int(frame_width * 0.6)
    roi_height_bottom = int(frame_height * 0.6)
    ...
    try:
        while True:
            ...
            if detection_result0['wood_candidates']:
                ...
                # Draw corner points
                corner_points = candidate['corner_points']
                cv2.drawContours(frame0, [corner_points], -1, (255, 0, 0), 2)
            ...
            # Draw bounding box on frame 2 (flipped) for the best candidate only
            if detection_result2['wood_candidates']:
                ...
                # Draw corner points
                corner_points = candidate['corner_points']
                cv2.drawContours(frame2_flipped, [corner_points], -1, (255, 0, 0), 2)
            ...
            # Resize frames and masks to 240p for display
            display_width, display_height = 426, 240  # 240p (16:9)
